...
class Swc:
    ...
    def save_file(self, filename):
        ...
        np.savetxt(filename, self.data,
                   fmt='%i %i %4.12f %4.12f %4.12f %4.5f %i')
    ...
    def scale(self, scale_factor, scale_radii=True):
        ...
        for node in range(0, self.num_nodes):
            ...
            if scale_radii:
                self.data[node, radius] = (self.data[node, radius] *
                                           scale_factor)

        self.surface_area = self.find_surface_area()

    def anisotropic_scale(self, x_scale, y_scale, z_scale, scale_radii=False):
        ...
        for node in range(0, self.num_nodes):
            ...
            self.data[node, x] = self.data[node, x] * x_scale
            self.data[node, y] = self.data[node, y] * y_scale
            self.data[node, z] = self.data[node, z] * z_scale
            if scale_radii:
                self.data[node, radius] = (self.data[node, radius] *
    ...
    def find_surface_area(self):
        """
        Models the swc file as a series of conical frustums to
    ...
    def adjust_surface_area(self, goal_surface_area, error_rate=0.001):
        ...
        This is done iteratively by scaling the radii,
        checking the surface area and then rescaling the radii until the
        surface area matches the goal area
        within the error rate (default=0.001)
        """
        radius = self.node_radius

        while np.abs((self.surface_area - goal_surface_area)
                     / goal_surface_area > error_rate):
            new_swc = self.data
            scaling_factor = goal_surface_area / self.surface_area
            for node in range(0, self.num_nodes):
                new_swc[node, radius] = (scaling_factor *
                                         self.data[node, radius])
    ...
    def rotate(self, x_rotation_deg, y_rotation_deg, z_rotation_deg):
        ...
        for node in range(0, self.num_nodes):
            ...
            self.data[node, y] = (self.data[node, y] * math.cos(x_rotation) -
                                  self.data[node, z] * math.sin(x_rotation)
                                  )
            self.data[node, z] = (self.data[node, y] * math.sin(x_rotation) +
                                  self.data[node, z] * math.cos(x_rotation)
                                  )

            # y rotations
            self.data[node, x] = (self.data[node, x] * math.cos(y_rotation) +
                                  self.data[node, z] * math.sin(y_rotation)
                                  )
            self.data[node, y] = self.data[node, y]
            self.data[node, z] = (self.data[node, z] * math.cos(y_rotation) -
                                  self.data[node, x] * math.sin(y_rotation)
                                  )

            # z rotations
            self.data[node, x] = (self.data[node, x] * math.cos(z_rotation) -
                                  self.data[node, y] * math.sin(z_rotation)
                                  )
            self.data[node, y] = (self.data[node, x] * math.sin(z_rotation) +
                                  self.data[node, y] * math.cos(z_rotation)
                                  )
